module.exports = (node, graph) => {
  ...
  function update() {
    ...
    if (inp.slug && inp.amount) {
      if (inp.amount <= spl) {
        left.setValue({slug: inp.slug, amount: inp.amount})
        right.setValue({slug: inp.slug, amount: 0})
        node.comment = "Not enough input for both sides!"
      } else {
        left.setValue({slug: inp.slug, amount: spl})
        right.setValue({slug: inp.slug, amount: inp.amount - spl})
        node.comment = `${left.value.amount}← ${resources[inp.slug]}/min →${right.value.amount}`
      }
    } else {
      left.setValue({})
      right.setValue({})
